...
typedef struct {
    ...
    unsigned char *zpng;
    unsigned int *buffer;
    std::pair<unsigned int,unsigned int> *frames;
    SubtractGreenFilter *filter;
...
prMALError SDKOpenFile8(imStdParms *stdParms, imFileRef *SDKfileRef, imFileOpenRec8 *SDKfileOpenRec8) {
    ...
    if(SDKfileOpenRec8->privatedata) {
        ...
        localRecP->frames = nullptr;
        localRecP->zpng = nullptr;
        localRecP->buffer = nullptr;
        localRecP->filter = nullptr;
    }
...
prMALError SDKGetInfo8(imStdParms *stdParms, imFileAccessRec8 *fileAccessInfo8, imFileInfoRec8 *SDKFileInfo8) {
    ...
    if(localRecP) {
                            ...
                            for(int k=0; k<(len-8)-3; k++) {
                                if(swapU32(*((unsigned int *)(moov+k)))==atom("stsz")) {
                                    ...
                                    if(k<(len-8)) {
                                        ...
                                        if(localRecP->frames) delete[] localRecP->frames;
                                        localRecP->frames = new std::pair<unsigned int,unsigned int>[frames];
                                        for(int f=0; f<frames; f++) {
                                            k+=4;
                                            if(k<len-8) {
                                                unsigned int size = swapU32(*((unsigned int *)(moov+k)));
                                                localRecP->frames[f] = std::make_pair(chunk,size);
                                                chunk+=size;
                                            }
                                        }
                                        break;
        ...
        if(frames>0) {
            ...
            if(localRecP->filter) {
                delete[]  localRecP->filter;
                localRecP->filter = nullptr;
            }
            
            localRecP->zpng = new unsigned char[localRecP->width*localRecP->height*4];
            localRecP->buffer = new unsigned int[localRecP->width*localRecP->height];
            localRecP->filter = new SubtractGreenFilter(localRecP->width,localRecP->height);
...
static prMALError SDKGetSourceVideo(imStdParms *stdParms, imFileRef fileRef, imSourceVideoRec *sourceVideoRec) {
    ...
    if(result!=suiteError_NoError) {
        ...
        if(frameFormat->inPixelFormat==PrPixelFormat_BGRA_4444_8u) {
            ...
            ByteCount actualCount = 0;
            unsigned int chunk = localRecP->frames[theFrame].first;
            unsigned int size = localRecP->frames[theFrame].second;
            FSReadFork((FSIORefNum)CAST_REFNUM(fileRef),fsFromStart,chunk,size,localRecP->zpng,&actualCount);
            ZSTD_decompress(localRecP->buffer,(w*h)<<2,localRecP->zpng,size);
            localRecP->filter->add(localRecP->buffer);
            for(int i=0; i<h; i++) {
                unsigned int *p = (unsigned int *)(pixelAddress+(i*rowBytes));
                for(int j=0; j<w; j++) {
                    unsigned int pixel = localRecP->buffer[i*w+j];
                    *p++ = (pixel&0xFF00FF00)|(pixel&0xFF)<<16|((pixel>>16)&0xFF);
                }
            }
